...
#ifdef _TEST_SERVER
	INT					m_nEft;			// 이펙트 변수 추가 by eriastoa 2006.05.29
#endif
...
protected:
	...
	VOID MonEffectLoad( WORD wDress, BYTE& bMtn, BYTE& bDir );		// 코드 정리 by eriastoa 2006.04.19
	BOOL SetMonImgIdx( WORD wDress );								// 코드 정리 by eriastoa 2006.04.20

	VOID DrawShadow( INT x, INT y, BOOL bShadowAblended, BYTE bShadowType, BYTE bOpaRate );	// 그림자 위치 조절 by eriastoa 2006.05.03
