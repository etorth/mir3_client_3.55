...
VOID CUtilWnd::SetMenuRect()
{
	...
	else if ( mUtil_nSeletedMenu >= 4 && mUtil_nSeletedMenu <= 13 )
	{
		for ( INT nI=0; nI<24; nI++ )
		{
			...
			mUtil_rcMenu[nI].top = m_rcWnd.top + 5 + 16*nI;
				...
				if ( mUtil_xUtilMagicInfo[nJ].nPriority == nI+1 )
				{
					if ( mUtil_xUtilMagicInfo[nJ].bCheck )
					{
						...
						if ( wTargetHP < mUtil_xUtilMagicInfo[nJ].nValue && bFindTarget && wTargetHP != 0 )
						{
							...
							{
								// 반월제외.
								if ( (10 + pstMagicRCD->stStdMagic.bDefSpell) <= g_xGameProc.m_xMyHero.m_stAbility.wMP || pstMagicRCD->stStdMagic.wMagicID == _SKILL_BANWOL || pstMagicRCD->stStdMagic.wMagicID == _SKILL_MANWOL || pstMagicRCD->stStdMagic.wMagicID == _SKILL_MANWOL_HIGH )
								{
...
{
	CHAR	szMagicName[16], szCharmName[16];
					...
					if ( strcmp( mUtil_szScroll, g_xGameProc.m_xInterface.m_xInventoryWnd.m_stInventoryItem[nCnt].xItem.m_stItemInfo.stStdItem.szName ) == 0 )
					{
						if ( !g_xGameProc.m_xInterface.m_stCommonItem.bSetted )
						{
							if ( g_xGameProc.m_bUseSpell == FALSE )
							{
								...
								g_xGameProc.m_xInterface.MsgAdd(dwFont, dwBack, g_xMsg.GetMsg(4890));
...
VOID CUtilWnd::UseMPPotion()
{
	...
	if ( mUtil_nMPValue > 0 && ( strcmp(mUtil_szMPpotion, g_xMsg.GetMsg(6070) ) == 0  || strcmp(mUtil_szHPpotion, g_xMsg.GetMsg(6071) ) == 0) )
	{
		CHAR szPotionName[MAX_PATH];
