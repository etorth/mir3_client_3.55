...
VOID CSprite::SetNPCFrameInfo(WORD wNPCNum)
{
	...
	//OldImageSetNPCFrameInfo( wNPCNum );
...
//		break;
//
//	case 154:	case 155:	case 162:	case 163:	case 208: // 원후군, 원후자, 지하여신1, 지하여신2, 설원늑대1
//		SetSprDfn(&m_stMonSpr[2],  160,  6, 120);
//		break;
...
//		break;
//	case 192:	case 191:	case 193:	case 194:	case 195:	case 196:	case 199:	case 204:	case 207:	case 171:
...
//	}
	...
	// 코드 정리 by eriastoa 2006.04.19
	OldImageSetMonFrameInfo( wMonNum );

	// 추가 이미지 ( 치우장군 ) by eriastoa 2006.04.16
	// 마소패, 마총패 by eriastoa 2006.04.18
	AddedImageSetMonFrameInfo( wMonNum );
}
...
// 추가 이미지
VOID
CSprite::OldImageSetNPCFrameInfo( WORD wMonNum )
{
	// 예전 이미지에 대한 NPC 프레임 정보 세팅, NPC 별로 정리함 by eriastoa 2006.04.19
	switch( wMonNum )
	{
	case 19:
		{
	...
	case 87:
		{
			SetSprDfn(&m_stNPCSpr[0], 0,  10, 150);		// 성훈씨 유휴동작
			SetSprDfn(&m_stNPCSpr[1], 0,  10, 150);		// 특수동작1
			SetSprDfn(&m_stNPCSpr[2], 0, 10,  150);		// 특수동작1
		}
		break;
...
VOID
CSprite::OldImageSetMonFrameInfo( WORD wMonNum )
{
	SetSprDfn(&m_stMonSpr[0],    0,  4, 300);		// 유휴동작
	SetSprDfn(&m_stMonSpr[1],    80,  6, 90);		// 걷기동작
	SetSprDfn(&m_stMonSpr[2],  160,  6, 130);		// 일반공격동작1
	SetSprDfn(&m_stMonSpr[3],  240, 2, 180);		// 맞기동작
	SetSprDfn(&m_stMonSpr[4],  320, 10, 140);		// 죽기동작
	SetSprDfn(&m_stMonSpr[5],  400,  6, 150);		// 일반공격동작2
	SetSprDfn(&m_stMonSpr[6],  480, 10, 150);		// 마법공격동작1
	SetSprDfn(&m_stMonSpr[7],  560, 10, 150);		// 마법공격동작2
	SetSprDfn(&m_stMonSpr[8],  640,  10, 120);		// 사라지기, 나타나기 동작
	SetSprDfn(&m_stMonSpr[9],  720,  6, 150);		// 특수동작1
	SetSprDfn(&m_stMonSpr[10],  880,  1, 500);		// 해골
	
	// 예전 이미지에 대한 몬스터 프레임 정보 세팅, 몬스터 별로 정리함 by eriastoa 2006.04.19
	switch( wMonNum )
	{
	case 0:		// 저파
		{
			SetSprDfn(&m_stMonSpr[0],    0,  4, 250);
			SetSprDfn(&m_stMonSpr[2],  160,  6, 100);
			SetSprDfn(&m_stMonSpr[3],  0, 4, 250);
			SetSprDfn(&m_stMonSpr[5],  400,  6, 150);
			SetSprDfn(&m_stMonSpr[7],  560, 10, 150);
			SetSprDfn(&m_stMonSpr[8],  640,  10, 120);
	...
	case 208:	// 앙코르 와트 설원늑대
		{
			SetSprDfn(&m_stMonSpr[2],  160,  6, 120);
		}
		break;

	case 211:	// 앙코르 와트 새벽여왕
		{
			SetSprDfn(&m_stMonSpr[5],  400,  9, 120);
		}
		break;
	...
	case 212:	// 앙코르 와트 가무녀
		{
			...
			SetSprDfn(&m_stMonSpr[2],  160,  10, 150);
			SetSprDfn(&m_stMonSpr[3],  240, 4, 150);
			SetSprDfn(&m_stMonSpr[4], 320, 10, 120 );		// 2006.07.06 추가
			SetSprDfn(&m_stMonSpr[5],  160,  10, 150);
		}
		break;

	case 213:	// 앙코르 와트 가무녀 2
		{
			SetSprDfn(&m_stMonSpr[0],    0, 10, 120);
			SetSprDfn(&m_stMonSpr[1],    80,  10, 50);
			SetSprDfn(&m_stMonSpr[2],  160,  20, 90);
...
// 추가 이미지
VOID
CSprite::AddedImageSetMonFrameInfo( WORD wMonNum )
{
}
