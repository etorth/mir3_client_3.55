...
{
	fnMakeDefMessage(&m_sendDefMsg, wIdent, nItemIdx, bEquipNum, 0, 0);
...
//	{
//		sprintf(g_xGameProc.m_szSocket, recvBuf);
//	}
...
VOID CClientSocket::OnChangeCharName(char *pszName)
{
	char szPacket[64];

	fnMakeDefMessage(&m_sendDefMsg, CM_CHANGENAME, 0, 0, 0, 0);

	wsprintf( szPacket, "%s/%d", pszName, g_nUpgradeLevel );
...
VOID CClientSocket::SendGameGuard(CHAR* pszData)
{
	fnMakeDefMessage(&m_sendDefMsg, CM_GAMEGUARD, 0, 0, 0, 0);
	SendPacket(&m_sendDefMsg, pszData);
}
