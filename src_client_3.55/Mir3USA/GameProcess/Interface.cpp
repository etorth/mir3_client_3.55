...
/*		if ( bStdMode != 0 || 
		...
		if ( (bStdMode != 0 && bStdMode != 3) ||
			((bStdMode == 0 || bStdMode == 3) && timeGetTime() - m_xInventoryWnd.m_dwDLClickTime > 2000) || // 물약류, 전서류...(2초지연)
			 (bStdMode == 0 && timeGetTime() - m_xInventoryWnd.m_dwDLClickTime > 1000 && (wLooks == 20 || wLooks == 21 || wLooks == 70) ) // 선화수등..(1초지연)
		   )
		{
...
VOID CInterface::SetViewMode()
{
	return;
#ifdef _NEWINTERFACE
	if ( m_xInventoryWnd.m_bActive && !m_xStatusWnd.m_bActive )
	{
	...
	else if ( !m_xInventoryWnd.m_bActive && m_xStatusWnd.m_bActive )
	{
