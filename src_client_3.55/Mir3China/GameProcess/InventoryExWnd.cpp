...
/******************************************************************************************************************
	...
	CInventoryExWnd Class

*******************************************************************************************************************/
/******************************************************************************************************************

	함수명 : CInventoryExWnd::CInventoryExWnd()
...
*******************************************************************************************************************/
CInventoryExWnd::CInventoryExWnd()
{
	Init();
}
...
VOID CInventoryExWnd::Init()
{
	...
	ZeroMemory(m_stInventoryItem, sizeof(INVENITEMSET)*_MAX_INVENEX_ITEM);
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::CreateInventoryWnd()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::CreateInventoryExWnd(INT nID, CWHWilImageData* pxWndImage, INT nFrameImgIdx, INT nStartX, INT nStartY, INT nWidth, INT nHeight, BOOL bCanMove)
{
	// 윈도우 생성.
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::SetInvenBtnInit()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::SetInvenBtnInit()
{
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::ShowInventoryWnd()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::ShowInventoryExWnd()
{
	// 윈도우 바탕그리기.
	ShowGameWnd();
...
//			m_xInventoryBtn[_BTN_ID_MINIMAPBOOK].ShowGameBtn();
			...
			g_xMainWnd.PutsHan(NULL, rcTitle, RGB(250, 120, 148), RGB(0, 0, 0), g_xMsg.GetMsg( 1701 ));

			// 추가 가방 무게 표시
			// 변수를 하나 더 선언해서 표시해야 한다
...
INT CInventoryExWnd::GetInvenCellNum(POINT ptMouse)
{
...
INT CInventoryExWnd::GetInvenItemNum(POINT ptMouse)
{
	INT		nCurrCell;
...
INT CInventoryExWnd::GetEmptyInvenNum()
{
	for ( INT nCnt = 0; nCnt < _MAX_INVENEX_ITEM; nCnt++ )
	{
...
BOOL CInventoryExWnd::CanItemInsert(INT nCellNum, CItem* pxItem, RECT& rcCell)
{
...
VOID CInventoryExWnd::SetItemState(CItem* pxItem, INT nItemNum, LPRECT lprcCell)
{
...
BOOL CInventoryExWnd::GetCellWH(WORD wLooks, INT& nCellWidth, INT& nCellHeight)
{
...
VOID CInventoryExWnd::ShowInvenItem()
{
...
//						
					...
					WORD wLooks = m_stInventoryItem[nCurrItmNum].xItem.m_stItemInfo.stStdItem.wLooks;
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::ShowInvenItemState()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::ShowInvenItemState()
{
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::DeleteInventoryCurrItem()
...
*******************************************************************************************************************/
BOOL CInventoryExWnd::DeleteInvenItem(INT nInvenItemNum)
{
	BOOL bRtn = FALSE;
...
BOOL CInventoryExWnd::DeleteInvenItem(INT nMakeIndex, CHAR* szName)
{
	BOOL bRtn = FALSE;
	for ( INT nCnt = 0; nCnt < _MAX_INVENEX_ITEM; nCnt++ )
	{
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::DeleteInventoryAllItem()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::DeleteAllInvenItem()
{
	for ( INT nCnt = 0; nCnt < _MAX_INVENEX_ITEM; nCnt++ )
	{
		DeleteInvenItem(nCnt);
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::AddInvenItem()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::AddInvenItem(CItem xItem, INT nCellNum, BOOL bUseInvenNum)
{
	RECT rcCell;
	if ( bUseInvenNum )
	{
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::ExtractInvenItem()
...
*******************************************************************************************************************/
BOOL CInventoryExWnd::ExtractInvenItem(CItem* pxItem, INT nInvenItemNum)
{
	if ( m_stInventoryItem[nInvenItemNum].bSetted )
	{
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::ChangeInvenWithCommonOLD()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::ChangeInvenWithCommonOLD(INT nInvenCellNum, INT nInvenItemNum, LPCOMMONITEMSET pstCommonItemSet)
{
	if ( pstCommonItemSet->bSetted )
	{
		// 추가 가방 2006.08.11
		if( pstCommonItemSet->bIsInvenWnd )
		{
		...
		if ( ExtractInvenItem(&xInvenItem, nInvenItemNum) )
		{
			...
			// 추가 가방 2006.08.11
			pstCommonItemSet->bIsInvenWnd		= FALSE;
			pstCommonItemSet->bIsInvenExWnd		= TRUE;
		}
	}
}

VOID CInventoryExWnd::ChangeInvenWithCommon(INT nInvenCellNum, INT nInvenItemNum, LPCOMMONITEMSET pstCommonItemSet)
{
	if ( pstCommonItemSet->bSetted )
	{
		// 추가 가방 2006.08.11
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::SetCommonFromInven()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::SetCommonFromInven(INT nInvenCellNum, INT nInvenItemNum, LPCOMMONITEMSET pstCommonItemSet)
{
	CItem xInvenItem;
	if ( ExtractInvenItem(&xInvenItem, nInvenItemNum) )
	{
...
VOID CInventoryExWnd::SetCommonFromGold(LPCOMMONITEMSET pstCommonItemSet)
{
	pstCommonItemSet->bSetted			= TRUE;
	pstCommonItemSet->bWaitResponse		= FALSE;
	pstCommonItemSet->bIsEquipItem		= FALSE;
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::SetInvenFromCommonOLD()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::SetInvenFromCommonOLD(INT nInvenCellNum, LPCOMMONITEMSET pstCommonItemSet)
{
	if ( pstCommonItemSet->bSetted )
	{
		// 추가 가방 2006.08.11
...
VOID CInventoryExWnd::SetInvenFromCommon(INT nInvenCellNum, LPCOMMONITEMSET pstCommonItemSet)
{
	if ( pstCommonItemSet->bSetted )
	{
		// 추가 가방 2006.08.11
...
LPINVENITEMSET CInventoryExWnd::FindInvenItem(WORD wLooks)
{
	for ( INT nCnt = 0; nCnt < _MAX_INVENEX_ITEM; nCnt++ )
	{
		if ( m_stInventoryItem[nCnt].bSetted )
...
VOID CInventoryExWnd::InitParkItem()
{
	m_nPrice = -1;
	ZeroMemory(m_pszPrice, 20);	
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::OnLButtonDown()
...
*******************************************************************************************************************/
BOOL CInventoryExWnd::OnLButtonDown(LPCOMMONITEMSET pstCommonItemSet, POINT ptMouse)
{
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::OnLButtonUp()
...
*******************************************************************************************************************/
BOOL CInventoryExWnd::OnLButtonUp(LPCOMMONITEMSET pstCommonItemSet, POINT ptMouse)
{
	// 더블클릭.
...
//	}
	...
	if ( m_bType == _INVEN_TYPE_BAG )
	{
		...
		if ( pstCommonItemSet->bSetted && pstCommonItemSet->bIsEquipItem && !pstCommonItemSet->bWaitResponse && !pstCommonItemSet->bIsDealItem && !pstCommonItemSet->bIsUtilItem )
		{
			if ( nInvenCellNum != -1 )
			{
				// g_xGameProc.m_xInterface.m_xStatusWnd.m_stTakeOnItemSet.bIsInvenExWnd 값에 다라서 어느 인벤토리로 내리느냐를 결정한다
				
				BOOL bTemp = g_xGameProc.m_xInterface.m_xStatusWnd.m_stTakeOnItemSet.bIsInvenExWnd;
				g_xGameProc.m_xInterface.m_xStatusWnd.m_stTakeOnItemSet.bIsInvenExWnd = TRUE;
				...
				g_xGameProc.m_xInterface.m_xStatusWnd.m_stTakeOnItemSet.bIsInvenExWnd = bTemp;
				return FALSE;
			}
		}
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::OnLButtonDoubleClick()
...
*******************************************************************************************************************/
BOOL CInventoryExWnd::OnLButtonDoubleClick(LPCOMMONITEMSET pstCommonItemSet, POINT ptMouse)
{	
	INT nInvenItemNum, nInvenCellNum;
...
*******************************************************************************************************************/
BOOL CInventoryExWnd::OnMouseMove(POINT ptMouse)
{
	for( INT nCnt = 0; nCnt < _MAX_INVEN_BTN; nCnt++ )
	{
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::OnScrollDown()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::OnScrollDown()
{
...
/******************************************************************************************************************
	...
	함수명 : CInventoryExWnd::OnScrollUp()
...
*******************************************************************************************************************/
VOID CInventoryExWnd::OnScrollUp()
{
...
BOOL CInventoryExWnd::OnRButtonDown(LPCOMMONITEMSET pstCommonItemSet, POINT ptMouse)
{
	RECT rcGold;
...
/*
#ifdef _TEST_SERVER_DEBUG	// 추가 창고 2006.08.09
			else if( m_bType == _INVEN_TYPE_STORAGEEX )
			{
				if (m_stInventoryItem[nItem].xItem.m_stItemInfo.stStdItem.bThrow >= _MULTIPLE_TYPE_ITEM)
				{
					DWORD	dwCount = m_stInventoryItem[nItem].xItem.m_stItemInfo.wAmount_GB;
					...
					g_xClientSocket.SendStorageItemEx(m_stInventoryItem[nItem].xItem.m_stItemInfo.nMakeIndex, m_nMerchantID, m_stInventoryItem[nItem].xItem.m_stItemInfo.stStdItem.szName);
...
VOID
CInventoryExWnd::ChangeItemCount(INT nIdx, DWORD wCount)
{
...
BOOL
CInventoryExWnd::IsThereMultipleItem(CItem xItem)
{
	for ( INT nCnt = 0; nCnt < _MAX_INVENEX_ITEM; nCnt++ )
	{
		if ( m_stInventoryItem[nCnt].bSetted )
...
INT
CInventoryExWnd::GetSameItemCount(CItem xItem)
{
	INT nCount = 0;
...
VOID
CInventoryExWnd::SetInvenType(BYTE bType)
{
	m_bType = bType;
