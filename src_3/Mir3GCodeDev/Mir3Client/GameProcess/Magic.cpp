						 ...
						 WORD wRed, WORD wGreen, WORD wBlue)
{
	...
	if ( pstMagicSpr )
	{
		...
		// 염색 마법들 칼라값 보정
		/*if ( nAtom != && wMagicNum == _SKILL_FIREBOOM )
		{
		...
		// 마석 착용에 의한 쾌도난마 이펙트 색 조정 2006.10.25
#ifdef _TEST_SERVER
		if ( wMagicNum == _SKILL_HYPERSWORD )
		{
			BYTE bMagicColor[ 4 ][ 3 ] = 
			{
			...
			if( g_xGameProc.m_nMagicStoneIdx > 0 )
			{
				nIndex = g_xGameProc.m_nMagicStoneIdx - 1;
			}
			else if( g_xGameProc.m_nMagicStoneIdx == -1 && g_xGameProc.m_xInterface.m_xStatusWnd.m_stEquipItem[_U_CHARM].xItem.m_stItemInfo.stStdItem.FuncType > 0 )	// 자신
			{
				nIndex = g_xGameProc.m_xInterface.m_xStatusWnd.m_stEquipItem[_U_CHARM].xItem.m_stItemInfo.stStdItem.FuncType - 1;				
			}

			if( nIndex != 255 )
			{
				m_bMagicColor[0] = bMagicColor[nIndex][0];
				m_bMagicColor[1] = bMagicColor[nIndex][1];
				m_bMagicColor[2] = bMagicColor[nIndex][2];
		...
		if ( m_bActiveState == _MAGIC_ACTIVE || m_bActiveState == _LIGHT_ACTIVE )
		{
			...
			else
			{
				...
				xLightFog->SetLightRadiusWithRing(nPosX+24,
							  					  nPosY+16, 
												  //xLightFog->SetLightRadiusWithRing(nPosX+100, nPosY+80, 
												  m_bLightRadius[0] + m_bCurrSwing, m_bLightColor[0][0], m_bLightColor[0][1], m_bLightColor[0][2],
												  m_bLightRadius[1] + m_bCurrSwing, m_bLightColor[1][0], m_bLightColor[1][1], m_bLightColor[1][2]);
			}
		}
...
//				D3DUtil_InitMaterial(mtrl, (FLOAT)((FLOAT)100/255.0f), (FLOAT)((FLOAT) 140/255.0f), (FLOAT)((FLOAT)190/255.0f));
//				D3DUtil_InitMaterial(mtrl, (FLOAT)((FLOAT) 97/255.0f), (FLOAT)((FLOAT) 167/255.0f), (FLOAT)((FLOAT)255/255.0f));
...
//				D3DUtil_InitMaterial(mtrl, (FLOAT)((FLOAT)240/255.0f), (FLOAT)((FLOAT)180/255.0f), (FLOAT)((FLOAT)100/255.0f));
				D3DUtil_InitMaterial(mtrl, (FLOAT)((FLOAT)m_bMagicColor[0]/255.0f), (FLOAT)((FLOAT)m_bMagicColor[1]/255.0f), (FLOAT)((FLOAT)m_bMagicColor[2]/255.0f));
				mtrl.diffuse.a = (m_bOpacity)/255.0f;
//				mtrl.diffuse.a = (20)/255.0f;
				g_xMainWnd.Get3DDevice()->SetMaterial(&mtrl);
	...
	case 1:															///     전개 프레임 처리
		{
			...
			if (mHurricaneMagic_nKind == _SKILL_HURRICANESHOT )
			{
				...
				float fDistance = sqrt((double)nOffY*nOffY + nOffX*nOffX), fTemp;
