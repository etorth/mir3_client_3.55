...
//  FindHack 관련
	...
	void				OnSocketMessageReceive(char *pszMsg, INT nLen);

	virtual VOID		OnConnectToServer();
	virtual char*		OnMessageReceive( char *pBuf, int nLen );

	INT					Encodebuffer(BYTE *szSource, BYTE *szKey, BYTE * szResult, BYTE nMode, int nSize);
