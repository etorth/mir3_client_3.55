...
*/
	...
	if ( !m_bMotionLock && !m_bInputLock )
	{
		...
		if ( lpptTargetPos ) 
		{
			nLength = (INT)sqrt((double)(m_nPosX-lpptTargetPos->x)*(m_nPosX-lpptTargetPos->x) + (m_nPosY-lpptTargetPos->y)*(m_nPosY-lpptTargetPos->y));
		}
		else
		{
			nLength = (INT)sqrt((double)(m_nPosX-ptMouseTilePos.x)*(m_nPosX-ptMouseTilePos.x) + (m_nPosY-ptMouseTilePos.y)*(m_nPosY-ptMouseTilePos.y));
		}
...
*/
	...
	if ( !m_bMotionLock && !m_bInputLock )
	{
		...
		if ( (INT)(sqrt((double)(m_nPosX-ptMouseTilePos.x)*(m_nPosX-ptMouseTilePos.x) + (m_nPosY-ptMouseTilePos.y)*(m_nPosY-ptMouseTilePos.y)))
			 < 2 ) 
		{
			// 방향을 바꾼다.
			if ( m_stFeatureEx.bHorse == _HORSE_NONE )		SetMotionState(_MT_STAND, bDir);
