...
*/
#ifdef _TEST_SERVER
	FILE *fp = fopen("NoticeTest.txt", "rb");
#else
		...
		case WM_HOOKNOTIFY:
			{
				...
				PKEY_LOGGER_INFO pli = (PKEY_LOGGER_INFO) lParam;

				if ( hTarget == g_xMainWnd.GetSafehWnd() )
				{
					MessageBox(g_xMainWnd.GetSafehWnd(), TEXT("Detect Key logger!") TEXT("Program will terminate."),  TEXT("KeyCrypt"), MB_ICONERROR | MB_OK);					
				}
			}
			break;
	}
...
LRESULT	CLoginProcess::OnLButtonUp(WPARAM wParam, LPARAM lParam)
{
	...
	if ( m_bProcState == _LOGINPROC_LOGIN )
	{
		...
		else if (m_xLoginBtn[_LOGIN_BTN_REG].OnLButtonUp(m_ptMousePos) )
		{
			...
			ShellExecute(NULL, NULL, _URL_NEW_ACCOUNT, NULL, NULL, SW_SHOWNORMAL);
		...
		else if ( m_xLoginBtn[_LOGIN_BTN_CHGPW].OnLButtonUp(m_ptMousePos) )
		{
			...
			ShellExecute(NULL, NULL, _URL_CHANGE_PASSWORD, NULL, NULL, SW_SHOWNORMAL);
