				   ...
				   FEATURE* pstFeature, FEATUREEX* pstFeatureEx)
{
	...
	pstFeature->bDress = pstFeature->bDress & 31; //이운
	//pstFeature->bDress = pstFeature->bDress & 15; // 2006.06.05 korea test server 는 15
...
VOID CHero::OnFeatureChanged(LPPACKETMSG lpPacketMsg)
{
	...
	stFeature.bDress	= stFeature.bDress & 31;
...
VOID CHero::OnMagicFire(LPPACKETMSG lpPacketMsg)
{
	...
	if ( wMagicID == _SKILL_SMALLSPACEMOVE ||
		 ...
		 wMagicID == _SKILL_POWERSKELLETON || wMagicID == _SKILL_FULLCLOAK   )
	{
		return;
	}
...
//	else if ( wMagicID == _SKILL_LIGHTFLOWER )							m_bEffectFrameCnt = 11;
	...
	switch( wMagicID )
	{
		case _SKILL_HYPERSWORD:
			SetMotionFrame(_MT_ONEHSWING, bMagicDir);
			break;
...
{
	stFeature.bDress = stFeature.bDress & 31; //이운교
...
/*
	if ( m_pxDragonImage )
	{
		if ( !m_pxDragonImage->NewSetIndex(m_dwCurrFrame) )
			return FALSE;
...
//		DrawWithEffected(m_rcDragon.left, m_rcDragon.top, m_pxDragonImage->m_lpstNewCurrWilImageInfo->shWidth, m_pxDragonImage->m_lpstNewCurrWilImageInfo->shHeight, (WORD*)m_pxDragonImage->m_pbCurrImage,
//						 m_stFeatureEx.wDressColor, 0XFFFF, bFocused, bOpaRate, wState);
	}
*/
...
VOID CMyHero::SetMagic(LPCLIENTMAGICRCD	pstMagic, BYTE bDir, INT nTargetID, FEATURE stTargetFeature, POINT ptTargetPos)
{
	...
	if ( !m_bMotionLock && !m_bInputLock )
	{
		switch ( m_nCurrMagicID )
		{
			case _SKILL_HYPERSWORD:
			{
				...
				SetMotionState(_MT_ONEHSWING, bDir);
				break;
			}
			case _SKILL_MOOTEBO:
	...
	case _SKILL_ERGUM_HIGH: case _SKILL_MANWOL_HIGH:	case _SKILL_AMYOUNSUL_HIGH:
	case _SKILL_REMOVEPOISON_HIGH:						case _SKILL_BALSACHE:	case _SKILL_JIKSUNGYE:		case _SKILL_JISOKGYE:
...
//			sprintf(szstr, "bMagicKey:%d, bKeyNum:%d, nMagicID:%d", bMagicKey, bKeyNum, nMagicID);
				...
				{
					// 반월제외.
					if ( ( pstMagic->stStdMagic.bDefSpell) <= m_stAbility.wMP || pstMagic->stStdMagic.wMagicID == _SKILL_BANWOL || pstMagic->stStdMagic.wMagicID == _SKILL_MANWOL || pstMagic->stStdMagic.wMagicID == _SKILL_MANWOL_HIGH )
					{
...
//			pstMagic = g_xGameProc.m_xInterface.m_xMagicWnd.GetMagicByKey(bMagicKey);
			...
			{
				// 반월제외.
				if ( ( pstMagic->stStdMagic.bDefSpell) <= m_stAbility.wMP || pstMagic->stStdMagic.wMagicID == _SKILL_BANWOL || pstMagic->stStdMagic.wMagicID == _SKILL_MANWOL )
				{
					m_nCurrMagicID = pstMagic->stStdMagic.wMagicID;
					SetMagic(pstMagic, bDir, nMosueTargetID, stTargetFeature, ptMouseTilePos);
				}
