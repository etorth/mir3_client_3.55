...
VOID CInterface::Init()
{
	...
	m_xMarketExWnd.Init();	// 원보상점
	m_xInventoryExWnd.Init();	// 추가 가방
...
//	m_xInterImgEx.Destroy();
	...
	m_xMarketExWnd.Destroy();	// 원보상점
	m_xInventoryExWnd.Destroy();	// 추가 가방
...
VOID CInterface::CreateInterface(CImageHandler* pxImgHandler)
{
	...
	// 원보상점
	m_xMarketExWnd.CreateMarketExWnd( _WND_ID_MARKETEX, m_pxInterImg, 3110, 50, 50, 720, 440, FALSE, 3 );	// 원보상점
	// 추가 가방
	m_xInventoryExWnd.CreateInventoryExWnd(_WND_ID_INVENTORYEX, m_pxInterImg, _WNDIMGIDX_INVENTORY, 418, 100, 282, 466, TRUE);
...
VOID CInterface::ShowWndList()
{
	...
	if ( m_xWndOrderList.GetCounter() != 0 )
	{
		...
		for ( INT nCnt = 0; nCnt < m_xWndOrderList.GetCounter(); nCnt++ )
		{
			...
			// 원보상점
			case _WND_ID_MARKETEX:
				m_xMarketExWnd.ShowMarketExWnd();
				prcInter = &m_xMarketExWnd.m_rcWnd;
				break;

			// 추가 가방
			case _WND_ID_INVENTORYEX:
				m_xInventoryExWnd.ShowInventoryExWnd();
				prcInter = &m_xInventoryExWnd.m_rcWnd;
				break;
			}			
		...
		// 추가 가방
		case _WND_ID_INVENTORYEX:
			m_xInventoryExWnd.ShowInvenItemState();
			break;
		}
...
//	WORD	wJobIcon;
	...
	INT		nDay	= g_xGameProc.m_nDayState;
	...
	// 하루의 상태치(밤, 낮, 저녁, 새벽)
	if ( nDay == g_xGameProc.m_nDuskSeed)
		wImgNum = 1201;
	else if ( nDay == g_xGameProc.m_nDaySeed)
		wImgNum = 1200;
	else if ( nDay == g_xGameProc.m_nDawnSeed)
		wImgNum = 1203;
	else if ( nDay == g_xGameProc.m_nNightSeed)
		wImgNum = 1202;
/*
	if (m_bIsLargeChat == TRUE)
	{
...
INT	CInterface::GetWindowInMousePos(POINT ptMouse)
{
	if ( m_xWndOrderList.GetCounter() != 0 )
	{
		...
		for ( INT nCnt = m_xWndOrderList.GetCounter()-1; nCnt >= 0 ; nCnt-- )
		{
			...
			// 원보상점
			case _WND_ID_MARKETEX:
				rcWnd = m_xMarketExWnd.GetGameWndRect();
				break;

			// 추가 가방
			case _WND_ID_INVENTORYEX:
				rcWnd = m_xInventoryExWnd.GetGameWndRect();
				break;
			}
...
//				m_xNPCWnd.CloseWnd();
	...
	case _WND_ID_MARKETEX:
		{
			if( m_xMarketExWnd.GetGameWndActive() )
			{
				DeleteWindowToList( _WND_ID_MARKETEX );
				m_xMarketExWnd.SetGameWndActive( FALSE );
				bRet = FALSE;
			}
			else
			{
				AddWindowToList( _WND_ID_MARKETEX );
				m_xMarketExWnd.SetGameWndActive( TRUE );
				bRet = TRUE;
			}
		}
		break;
	...
	case _WND_ID_INVENTORYEX:
		{
			if ( m_xInventoryExWnd.GetGameWndActive() )
			{
				DeleteWindowToList(_WND_ID_INVENTORYEX);
				m_xInventoryExWnd.SetGameWndActive(FALSE);
				bRet = FALSE;
			}
			else
			{
				AddWindowToList(_WND_ID_INVENTORYEX);
				m_xInventoryExWnd.SetGameWndActive(TRUE);
				m_xInventoryExWnd.SetInvenType(_INVEN_TYPE_BAG);
				m_xInventoryExWnd.InitParkItem();
				bRet = TRUE;
			}
		}
		break;
...
VOID CInterface::MoveTopWindow(POINT ptMouse)
{
	...
	if ( m_xWndOrderList.GetCounter() != 0 )
	{
		...
		if ( m_bMoveFocusedWnd )
		{
			...
			// 추가 가방
			case _WND_ID_INVENTORYEX:
				m_xInventoryExWnd.MoveGameWnd( ptMouse );
				break;
			}
		}
	}
...
VOID CInterface::WindowFocusChangedAndSetReadyMove(INT nID, POINT ptMouse)
{
	if ( m_xWndOrderList.GetCounter() != 0 )
	{
		...
		// 원보상점
		case _WND_ID_MARKETEX:
			m_xMarketExWnd.SetGameWndFocused( TRUE );
			m_xMarketExWnd.SetReadyMove( ptMouse );
			break;

		// 추가 가방
		case _WND_ID_INVENTORYEX:
			m_xInventoryExWnd.SetGameWndFocused( TRUE );
			m_xInventoryExWnd.SetReadyMove( ptMouse );
			break;
		}
	}	
}
...
VOID CInterface::AllWindowsFocusOff()
{
	if ( m_xWndOrderList.GetCounter() != 0 )
	{
		...
		for ( INT nCnt = 0; nCnt < m_xWndOrderList.GetCounter(); nCnt++ )
		{
			...
			// 원보상점
			case _WND_ID_MARKETEX:
				m_xMarketExWnd.SetGameWndFocused( FALSE );
				break;

			// 추가 가방
			case _WND_ID_INVENTORYEX:
				m_xInventoryExWnd.SetGameWndFocused( FALSE );
				break;
			}
		}
	}
...
*/
	...
	if ( nInterfaceWndInRect != -1 )
	{
		...
		// 추가 가방
		case _WND_ID_INVENTORYEX:
			if( m_xInventoryExWnd.OnRButtonDown( &m_stCommonItem, ptMouse ) )
				return TRUE;
			break;
		}
	}
...
BOOL CInterface::OnLButtonDown(POINT ptMouse)
{
	...
	if ( nInterfaceWndInRect != -1 )
	{
		...
		// 원보상점
		case _WND_ID_MARKETEX:			
			if( ! m_xMarketExWnd.OnLButtonDown( &m_stCommonItem, ptMouse ) )
				WindowFocusChangedAndSetReadyMove( nInterfaceWndInRect, ptMouse );
			break;

		// 추가 가방
		case _WND_ID_INVENTORYEX:
			if( ! m_xInventoryExWnd.OnLButtonDown( &m_stCommonItem, ptMouse ) )
				WindowFocusChangedAndSetReadyMove( nInterfaceWndInRect, ptMouse );
			break;
		}
...
BOOL CInterface::OnLButtonUp(POINT ptMouse)
{
	...
	if ( nInterfaceWndInRect != -1 )
	{
		switch ( nInterfaceWndInRect )
		{
			...
			case _WND_ID_STORE:			
			{
				if ( m_xStoreWnd.OnLButtonUp(ptMouse) )							
				{
					WindowActivate(_WND_ID_STORE);			
				}
				break;
			}
			...
			case _WND_ID_NPCCHAT:		
			{
				if ( m_xNPCWnd.OnLButtonUp(ptMouse) )	
				{
			...
			// 원보상점
			case _WND_ID_MARKETEX:
				if( m_xMarketExWnd.OnLButtonUp( &m_stCommonItem, ptMouse ) ) WindowActivate( _WND_ID_MARKETEX );
				break;

			// 추가 가방
			case _WND_ID_INVENTORYEX:
				if( m_xInventoryExWnd.OnLButtonUp( &m_stCommonItem, ptMouse ) && CanViewModeChange() )
				{
					WindowActivate(_WND_ID_INVENTORYEX);		
					SetViewMode();
				}
				break;
		}
...
{
	// 유료 아이템들의 정보를 나타낸다
	m_bDrawExpireAbilitys = FALSE;
	if ( PtInRect(&m_rcLogo, ptMouse) && m_nInformationMode != _INFO_CHAR )
	{
		m_bDrawExpireAbilitys = TRUE;
	}
	...
	if ( nInterfaceWndInRect != -1 )
	{
		...
		// 원보상점
		case _WND_ID_MARKETEX:
			m_xMarketExWnd.OnMouseMove( ptMouse );
			break;

		// 추가 가방
		case _WND_ID_INVENTORYEX:
			if( m_xInventoryExWnd.OnMouseMove( ptMouse ) )
				return TRUE;
		}
	}
	else
	{
		...
		// 추가 가방
		m_xInventoryExWnd.SetInvenBtnInit();
	}
...
BOOL CInterface::OnScrollDown()
{
	...
	if ( nInterfaceWndInRect != -1 )
	{
		...
		// 원보상점
		case _WND_ID_MARKETEX:
			m_xMarketExWnd.OnScrollDown();
			break;

		// 추가 가방
		case _WND_ID_INVENTORYEX:
			m_xInventoryExWnd.OnScrollDown();
			break;
		}
	}	
	else
...
BOOL CInterface::OnScrollUp()
{
	...
	if ( nInterfaceWndInRect != -1 )
	{
		...
		// 원보상점
		case _WND_ID_MARKETEX:
			m_xMarketExWnd.OnScrollUp();
			break;

		// 추가 가방
		case _WND_ID_INVENTORYEX:
			m_xInventoryExWnd.OnScrollUp();
			break;
		}
	}
	else
...
/*		if ( bStdMode != 0 || 
		...
		if ( (bStdMode != 0 && bStdMode != 3) ||
			((bStdMode == 0 || bStdMode == 3) && timeGetTime() - m_xInventoryWnd.m_dwDLClickTime > 2000) || // 물약류, 전서류...(2초지연)
			 ...
			 (wLooks == 20 || wLooks == 21 || wLooks == 70
			 || wLooks == 25 || wLooks == 26 || wLooks == 27 || wLooks == 28 || wLooks == 35 || wLooks == 36 || wLooks == 37 || wLooks == 38
...
#ifdef _NEWINTERFACE
	// 추가 가방
	if ( m_xInventoryWnd.m_bActive && !m_xStatusWnd.m_bActive || m_xInventoryExWnd.m_bActive && !m_xStatusWnd.m_bActive )
	{
		g_xGameProc.m_bViewMode	= _VIEW__WITH_INVEN;
		g_xGameProc.m_nViewRGapX = _VIEW__INVEN_GAP;
		g_xGameProc.m_xMap.m_nViewPosX = _VIEW_CELL_X_START_INVEN;
		g_xGameProc.m_xMap.m_nStartViewTileX += g_xGameProc.m_nViewRGapX;
		g_xGameProc.m_nViewLGapX = 0;
	}
	else if ( !m_xInventoryWnd.m_bActive && m_xStatusWnd.m_bActive || !m_xInventoryExWnd.m_bActive && m_xStatusWnd.m_bActive )
	{
		g_xGameProc.m_bViewMode	= _VIEW__WITH_STATUS;
		g_xGameProc.m_nViewLGapX = _VIEW__STATUS_GAP;
		g_xGameProc.m_xMap.m_nViewPosX = _VIEW_CELL_X_START_STATUS;
		g_xGameProc.m_xMap.m_nStartViewTileX += g_xGameProc.m_nViewLGapX;
		g_xGameProc.m_nViewRGapX = 0;
	}
	else
	{
		g_xGameProc.m_bViewMode	= _VIEW__WITH_NORMAL;
		g_xGameProc.m_xMap.m_nStartViewTileX -= (g_xGameProc.m_nViewLGapX + g_xGameProc.m_nViewRGapX);
		g_xGameProc.m_xMap.m_nViewPosX = _VIEW_CELL_X_START;
		g_xGameProc.m_nViewLGapX = 0;
		g_xGameProc.m_nViewRGapX = 0;
	}

	g_xGameProc.m_xMap.LoadNewMapBuffer();
#endif
}
...
BOOL CInterface::OnKeyDown(WPARAM wParam, LPARAM lParam)
{
	...
	// 원보상점
	if( m_xMarketExWnd.GetGameWndActive() && IsTopWindow( _WND_ID_MARKETEX ) )
	{
		if( m_xMarketExWnd.OnKeyDown( wParam, lParam ) )
			return TRUE;
	}
...
//		if ( GetKeyState(VK_CONTROL) & 0x80 )
		{
			if ( CanViewModeChange() )
			{
				...
				else if( ! m_xInventoryExWnd.GetGameWndActive() )
				{
					// 추가 가방
					WindowActivate( _WND_ID_INVENTORYEX );
					SetViewMode();
				...
				else if( m_xInventoryWnd.GetGameWndActive() && m_xInventoryExWnd.GetGameWndActive() )
				{
					WindowActivate(_WND_ID_INVENTORY);
					WindowActivate(_WND_ID_INVENTORYEX);
				}
...
VOID
// 유료 아이템 정보 세팅
CInterface::SetExpireAbilitys()
{
	// m_TExpireAbilitys -> m_TExpireState	

	INT nYear, nMonth, nDay, nHour, nMin;
	m_nExpireCnt = 0;

	// 추가 창고
	if( m_TExpireAbilitys.nSavedItemExExpireTime != 0 )
	{
		GetExpireTime( m_TExpireAbilitys.nSavedItemExExpireTime, nYear, nMonth, nDay, nHour, nMin );
		
		/*if( nYear == -1 )
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7210 ), m_TExpireAbilitys.nSavedItemExExpireTime );
		else*/
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7200 ), nYear, nMonth, nDay, nHour, nMin );
	}

	// 추가 가방
	if( m_TExpireAbilitys.nItemExExpireTime != 0 )
	{
		GetExpireTime( m_TExpireAbilitys.nItemExExpireTime, nYear, nMonth, nDay, nHour, nMin );

		/*if( nYear == -1 )
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7211 ), m_TExpireAbilitys.nItemExExpireTime );
		else*/
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7201 ), nYear, nMonth, nDay, nHour, nMin );
	}	

	// 유저 타이틀
	if( m_TExpireAbilitys.nUserTitleExprieTime != 0 )
	{
		GetExpireTime( m_TExpireAbilitys.nUserTitleExprieTime, nYear, nMonth, nDay, nHour, nMin );

		/*if( nYear == -1 )
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7212 ), m_TExpireAbilitys.nUserTitleExprieTime );
		else*/
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7202 ), nYear, nMonth, nDay, nHour, nMin );
	}	

	// 경험치
	if( m_TExpireAbilitys.nExpRateExpireTime != 0 )
	{
		GetExpireTime( m_TExpireAbilitys.nExpRateExpireTime, nYear, nMonth, nDay, nHour, nMin );

		/*if( nYear == -1 )
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7213 ), m_TExpireAbilitys.nExpRateExpireTime );
		else*/
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7203 ), m_TExpireAbilitys.fExpRate, nYear, nMonth, nDay, nHour, nMin );
	}	

	// 드랍율
	if( m_TExpireAbilitys.nDropRateExpireTime != 0 )
	{
		GetExpireTime( m_TExpireAbilitys.nDropRateExpireTime, nYear, nMonth, nDay, nHour, nMin );

		/*if( nYear == -1 )
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7214 ), m_TExpireAbilitys.nDropRateExpireTime );
		else*/
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7204 ), m_TExpireAbilitys.fDropRate, nYear, nMonth, nDay, nHour, nMin );
	}	

	// 채광율
	if( m_TExpireAbilitys.nMiningRateExpireTime != 0 )
	{
		GetExpireTime( m_TExpireAbilitys.nMiningRateExpireTime, nYear, nMonth, nDay, nHour, nMin );

		/*if( nYear == -1 )
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7215 ), m_TExpireAbilitys.nMiningRateExpireTime );
		else*/
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7205 ), m_TExpireAbilitys.fMiningRate, nYear, nMonth, nDay, nHour, nMin );
	}	

	// 몬스터 레벨 무시
	if( m_TExpireAbilitys.nIgnoreLevelExpireTime != 0 )
	{
		GetExpireTime( m_TExpireAbilitys.nIgnoreLevelExpireTime, nYear, nMonth, nDay, nHour, nMin );

		/*if( nYear == -1 )
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7216 ), m_TExpireAbilitys.nIgnoreLevelExpireTime );
		else*/
			sprintf( m_TExpireState[ m_nExpireCnt++ ].pszDesc, g_xMsg.GetMsg( 7206 ), nYear, nMonth, nDay, nHour, nMin );
	}
}

VOID
// 유료 아이템 만료 기간 계산
CInterface::GetExpireTime( INT nExpireTime, INT& nYear, INT& nMonth, INT& nDay, INT& nHour, INT& nMin )
{
	// 아직 적용되지 않은 아이템
	// 먹는 시점에 적용되는 아이템의 먹었을 경우의 적용 시간을 나타낸다
	if( nExpireTime > 0 && nExpireTime < 10000 )
	{
		nYear = -1;
	}
	else
	{
		nYear	= nExpireTime / 100000000;
		nMonth	= ( nExpireTime - 100000000 * nYear ) / 1000000;
		nDay	= ( nExpireTime - 100000000 * nYear - 1000000 * nMonth ) / 10000;
		nHour	= ( nExpireTime - 100000000 * nYear - 1000000 * nMonth - 10000 * nDay ) / 100;
		nMin	= ( nExpireTime - 100000000 * nYear - 1000000 * nMonth - 10000 * nDay - 100 * nHour );
...
VOID
// 유료 아이템 정보 보기
CInterface::ShowExpireAbilitys()
{
	INT n = 0;
	// 전체 판 크기 계산
	SIZE size = { 0, 0 }, board = { 0, 0 };
	for( n = 0; n < m_nExpireCnt; ++n )
	{
		size = g_xMainWnd.GetStrLength( NULL, NULL, m_TExpireState[ n ].pszDesc );
		if( size.cx > board.cx )
		{
			board.cx = size.cx;
		}
		...
		m_TExpireState[ n ].dwBack = RGB( 0, 0, 0 );
		m_TExpireState[ n ].dwFont = RGB( 255, 240, 150 );
	}

	//RECT rcB = { 100, 100, 100 + board.cx, 100 + board.cy };
	RECT rcB;
	rcB.left = 50;
	rcB.top = 600 - board.cy - 70;
	rcB.right = 50 + board.cx + 10;
	rcB.bottom = 600 - 70 + 5;
	INT nX = rcB.right - rcB.left;
	INT nY = rcB.bottom - rcB.top;
	RECT rc = rcB;
	if ( rcB.right > 800 )
	{
	...
	// 뒷판 그리기.
	D3DVECTOR	 vecTrans((FLOAT)(rc.left), (FLOAT)(rc.top), 0);
	D3DVECTOR	 vecScale((FLOAT)(nX), (FLOAT)(nY), 1);
	D3DMATERIAL7 mtrl;

	D3DUtil_InitMaterial(mtrl, (FLOAT)140/255.0f, (FLOAT)100/255.0f, (FLOAT)50/255.0f);
	mtrl.diffuse.a = 230.0f/255.0f;
	g_xGameProc.m_xImage.DrawBillBoard(g_xMainWnd.Get3DDevice(), &vecTrans, &vecScale, &mtrl, NULL);

	g_xMainWnd.DrawWithGDI(&rc, NULL, RGB(180, 180, 180), 1);

	// 유료 아이템 정보 텍스트
	for ( n = 0; n < m_nExpireCnt; ++n )
	{
		g_xMainWnd.PutsHan(NULL, rc.left + 5, rc.top + 5 + n * 15, m_TExpireState[ n ].dwFont, m_TExpireState[ n ].dwBack, m_TExpireState[ n ].pszDesc );	
	}
}