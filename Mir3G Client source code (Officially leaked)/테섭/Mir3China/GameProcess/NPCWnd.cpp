...
#define MAX_SHOW_LINE_NO_IMG	21
#define POS_DRAWSTART_Y			40
#define LINE_GAP				4		

#define BUILD_GUILD				"@@BuildGuild"			// 문파 설립
			...
			case SCRIPT_GENTEXT:
			{
				...
				if (nLine + tSize.cx > nLengthWidth)
				{
					...
					for (int i = 0; i < nLen; i++)
					{
						...
						{
							if (pScriptLine->m_pszScriptText[i] == ' ')
								int nI = 0;
		...
		if (pScriptLine->m_btScriptType == SCRIPT_BUTTON)
		{
			...
			if (pScriptLine->m_fIsSelected == TRUE && timeGetTime()-m_dwLastNPCTick > 1000 )
			{
				if (pScriptLine->m_pszScriptCommand[0] == '@' && pScriptLine->m_pszScriptCommand[1] == '@' && pScriptLine->m_pszScriptCommand[2] == '*')
				{
					...
					if (strcmp(pScriptLine->m_pszScriptCommand, "@@*BuildGuild") == 0)
					{
				...
				else if (pScriptLine->m_pszScriptCommand[0] == '@' && pScriptLine->m_pszScriptCommand[1] == '@')
				{
					...
					if (strcmp(pScriptLine->m_pszScriptCommand, "@@BuildGuild") == 0)
					{
						g_xGameProc.m_xGameMsgBox.CreateMsgBox(&g_xGameProc.m_xImage.m_xImageList[_IMAGE_INTER], _WND_ID_NPCCHAT, _YESNO, g_xMsg.GetMsg(2000), TRUE, -1, -1, CM_MERCHANTDLGSELECT, FALSE, 20);
					}
					else if (strcmp(pScriptLine->m_pszScriptCommand, "@@guildwar") == 0)
					{
						g_xGameProc.m_xGameMsgBox.CreateMsgBox(&g_xGameProc.m_xImage.m_xImageList[_IMAGE_INTER], _WND_ID_NPCCHAT, _YESNO, pScriptLine->m_pszScriptText, TRUE, -1, -1, CM_MERCHANTDLGSELECT, FALSE, 20);
					}
					else
					{
