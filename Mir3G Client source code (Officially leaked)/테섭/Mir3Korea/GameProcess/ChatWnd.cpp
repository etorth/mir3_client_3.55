...
VOID CChatWnd::Init()
{
	...
	mChat_nFstLineInAll		= 0;
	mChat_nMaxLineInAll	= 0;

	SetRect(&mChat_rcChat, 0, 0, 0, 0);

	mChat_xChatlist.ClearAll();
//	mChat_xNormalChatlist.ClearAll();
//	mChat_xGroupChatlist.ClearAll();
//	mChat_xGuildChatlist.ClearAll();
...
VOID CChatWnd::ShowChatWnd()
{
	if ( mChat_bLargeChat )
	{
		mChat_nMaxLineInAll = mChat_nNumMiddleImg + 3;
		...
		if ( mChat_nFstLineInAll == mChat_xChatlist.GetCount() )
		{
			mChat_nFstLineInAll -= 1;
		}

		mChat_xScrlChat.ShowScrlBar(m_rcWnd.left + 342, m_rcWnd.top + 22, mChat_nFstLineInAll, mChat_xChatlist.GetCount());
...
BOOL CChatWnd::OnLButtonDown(POINT ptMouse)
{
	...
	if ( mChat_xScrlChat.OnLButtonDown(ptMouse) )
	{
		...
		if ( mChat_nShowMode == _SHOW_ALLCHAT )
		{
			fScrlRate = mChat_xScrlChat.GetScrlRate();

			mChat_nFstLineInAll = (INT)((mChat_xChatlist.GetCount()-1)*fScrlRate);
		}
/*		else if ( mChat_nShowMode == _SHOW_EXCEPTSYS )
		{
			fScrlRate = mChat_xScrlChat.GetScrlRate();

			mChat_nFstLineInNormal = (INT)((mChat_xNormalChatlist.GetCount()-1)*fScrlRate);
		}
		else if ( mChat_nShowMode == _SHOW_SHOUTONLY )
		{
			fScrlRate = mChat_xScrlChat.GetScrlRate();

			mChat_nFstLineInShout = (INT)((mChat_xShoutChatlist.GetCount()-1)*fScrlRate);
		}
		else if ( mChat_nShowMode == _SHOW_GUILDONLY )
		{
			fScrlRate = mChat_xScrlChat.GetScrlRate();

			mChat_nFstLineInGroup = (INT)((mChat_xGroupChatlist.GetCount()-1)*fScrlRate);
		}
		else if ( mChat_nShowMode == _SHOW_GROUPONLY )
		{
			fScrlRate = mChat_xScrlChat.GetScrlRate();

			mChat_nFstLineInGuild = (INT)((mChat_xGuildChatlist.GetCount()-1)*fScrlRate);
		}
*/
		return TRUE;
	}
...
VOID CChatWnd::OnScrollDown()
{
	if ( mChat_nShowMode == _SHOW_ALLCHAT )
	{
		if ( mChat_nFstLineInAll > 0 )
			mChat_nFstLineInAll--;
	}
	else if ( mChat_nShowMode == _SHOW_EXCEPTSYS )
	{
		if ( mChat_nFstLineInNormal > 0 )
			mChat_nFstLineInNormal--;
	}
	else if ( mChat_nShowMode == _SHOW_SHOUTONLY )
	{
		if ( mChat_nFstLineInShout > 0 )
			mChat_nFstLineInShout--;
	}
	else if ( mChat_nShowMode == _SHOW_GUILDONLY )
	{
		if ( mChat_nFstLineInGroup > 0 )
			mChat_nFstLineInGroup--;
	}
	else if ( mChat_nShowMode == _SHOW_GROUPONLY )
	{
		if ( mChat_nFstLineInGuild > 0 )
			mChat_nFstLineInGuild--;
	}
}
...
VOID CChatWnd::OnScrollUp()
{
	if ( mChat_nShowMode == _SHOW_ALLCHAT )
	{
		if ( mChat_nFstLineInAll < (mChat_xChatlist.GetCount()-1) )
			mChat_nFstLineInAll++;
	}
/*	else if ( mChat_nShowMode == _SHOW_EXCEPTSYS )
	{
		if ( mChat_nFstLineInNormal < (mChat_xNormalChatlist.GetCount()-1) )
			mChat_nFstLineInNormal++;
	}
	else if ( mChat_nShowMode == _SHOW_SHOUTONLY )
	{
		if ( mChat_nFstLineInShout < (mChat_xShoutChatlist.GetCount()-1) )
			mChat_nFstLineInShout++;
	}
	else if ( mChat_nShowMode == _SHOW_GUILDONLY )
	{
		if ( mChat_nFstLineInGroup < (mChat_xGroupChatlist.GetCount()-1) )
			mChat_nFstLineInGroup++;
	}
	else if ( mChat_nShowMode == _SHOW_GROUPONLY )
	{
		if ( mChat_nFstLineInGuild < (mChat_xGuildChatlist.GetCount()-1) )
			mChat_nFstLineInGuild++;
	}
*/
}
...
VOID CChatWnd::MsgAdd(DWORD dwFontColor, DWORD dwFontBackColor, CHAR* pszMsg)
{
	if ( pszMsg )
	{
		if ( pszMsg[0] != NULL )
		{
			...
			{
				if ( mChat_xChatlist.GetCount() - mChat_nFstLineInAll == mChat_nMaxLineInAll )
				{
					mChat_nFstLineInAll++;
				}
...
VOID CChatWnd::ShowChatList()
{
	if ( !mChat_xChatlist.IsEmpty() )
	{
		...
		pNode = mChat_xChatlist.GetHead();
		for ( INT nCount=0; nCount < mChat_nFstLineInAll; nCount++)
			pNode = pNode->GetNext();

		nShowLine = mChat_xChatlist.GetCount() - mChat_nFstLineInAll;
	...
	for ( nCnt = 0; nCnt < nChatMaxLine; nCnt++ )
	{
		...
		if ( bInRect == TRUE && nNum >= 0)
		{
			...
			if ( !bEnpty )
			{
				...
				pNode = mChat_xChatlist.GetHead();
				for ( INT nCount=0; nCount < mChat_nFstLineInAll+nNum && pNode != NULL ; nCount++)
					pNode = pNode->GetNext();
