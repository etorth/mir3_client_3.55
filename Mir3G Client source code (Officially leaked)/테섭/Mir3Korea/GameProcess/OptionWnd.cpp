...
VOID COptionWnd::Init()
{
	...
	// 마법지속시간 표시
#ifdef _TEST_SERVER
	mOptionWnd_bShowMagicEffectTime = FALSE;
#endif
}
...
//	m_xOptionBtn[_BTN_ID_SHOWHEALTHNUM].CreateGameBtn  (pxWndImage, 1292, 1293, nStartX+448, nStartY+359, "", _BTN_TYPE_FOCUS, 1292, TRUE);
	...
	// 마법지속시간 표시
#ifdef _TEST_SERVER
	m_xOptionBtn[ _BTN_ID_SHOWMAGICEFFECTTIME ].CreateGameBtn( pxWndImage, 297, 297, nStartX + 448, nStartY + 359, "", _BTN_TYPE_FOCUS, -1, TRUE );
#endif
...
//		g_xGameProc.m_xInterface.m_xInterBtn[ _NEW_BTN_ID_DENYSHOUT].ChangeBtnTooltip(g_xMsg.GetMsg(2644));
	...
	// 마법지속시간 표시
#ifdef _TEST_SERVER
	if( mOptionWnd_bShowMagicEffectTime )
	{
		m_xOptionBtn[ _BTN_ID_SHOWMAGICEFFECTTIME ].ResetGameBtn( 297, 297, -1 );
	}
	else
	{
		m_xOptionBtn[ _BTN_ID_SHOWMAGICEFFECTTIME ].ResetGameBtn( 301, 301, 299 );
	}
#endif
/*
...
VOID COptionWnd::ShowVisualSet()
{
	...
	// 마법지속시간 표시
#ifdef _TEST_SERVER
	m_xOptionBtn[ _BTN_ID_SHOWMAGICEFFECTTIME ].ChangeRect( m_rcWnd.left + 209, m_rcWnd.top + 318 );
	m_xOptionBtn[ _BTN_ID_SHOWMAGICEFFECTTIME ].ShowGameBtn();
#endif
	...
	// 마법지속시간 표시
#ifdef _TEST_SERVER
	sprintf( pszTmp, g_xMsg.GetMsg( 2165 ) );
	...
	case _VISUAL_SET:
		{
			...
			// 마법지속시간 표시
#ifdef _TEST_SERVER
			 else if( m_xOptionBtn[ _BTN_ID_SHOWMAGICEFFECTTIME ].OnLButtonUp( ptMouse ) )
			 {
				 DWORD dwFont = GetChatColor(_CHAT_COLOR3);
				 DWORD dwBack = GetChatColor(_CHAT_COLOR8);
				
				 mOptionWnd_bShowMagicEffectTime = ! mOptionWnd_bShowMagicEffectTime;
				 if( mOptionWnd_bShowMagicEffectTime )
				 {
					 g_xGameProc.m_xInterface.MsgAdd(dwFont, dwBack, g_xMsg.GetMsg(2166));
					 m_xOptionBtn[_BTN_ID_SHOWMAGICEFFECTTIME].ResetGameBtn(297, 297, -1);
				 }
				 else
				 {
					 g_xGameProc.m_xInterface.MsgAdd(dwFont, dwBack, g_xMsg.GetMsg(2167));
					 m_xOptionBtn[_BTN_ID_SHOWMAGICEFFECTTIME].ResetGameBtn(301, 301, 299);					 
				 }
				 SetIniFile();
			 }
...
BOOL COptionWnd::OnLButtonDown(POINT ptMouse)
{
	...
	case _VISUAL_SET:
		...
		// 마법지속시간 표시
#ifdef _TEST_SERVER
		if( m_xOptionBtn[ _BTN_ID_SHOWMAGICEFFECTTIME ].OnLButtonDown( ptMouse ) )
			return TRUE;
#endif
		break;
	}
...
VOID COptionWnd::SetIniFile()
{
	...
	if ( GetCurrentDirectory(MAX_PATH, szFileName))
	{
		...
		// 마법지속시간 표시
#ifdef _TEST_SERVER
		_itoa( mOptionWnd_bShowMagicEffectTime, szVal, 10 );
		bResult = WritePrivateProfileString( SECTION_OPTIONS, KEY_SHOWMAGICEFFECTTIME, szVal, szFileName );
#endif
...
//		g_xGameProc.m_xInterface.m_nInformationMode = atoi(szTemp);
		...
		// 마법지속시간 표시
#ifdef _TEST_SERVER
		GetPrivateProfileString( SECTION_OPTIONS, KEY_SHOWMAGICEFFECTTIME, "0", szTemp, MAX_PATH, szFileName );
		if( szTemp == NULL )
			return;
		mOptionWnd_bShowMagicEffectTime = atoi( szTemp );
#endif
