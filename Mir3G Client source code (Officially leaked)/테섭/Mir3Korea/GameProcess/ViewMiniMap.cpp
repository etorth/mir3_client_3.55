				...
				if ( mUtil_xUtilMagicInfo[nJ].nPriority == nI+1 )
				{
					if ( mUtil_xUtilMagicInfo[nJ].bCheck )
					{
						...
						if ( wTargetHP < mUtil_xUtilMagicInfo[nJ].nValue && bFindTarget)
						{
							...
							{
								// 반월제외.
								if ( (10 + pstMagicRCD->stStdMagic.bDefSpell) <= g_xGameProc.m_xMyHero.m_stAbility.wMP || pstMagicRCD->stStdMagic.wMagicID == _SKILL_BANWOL || pstMagic->stStdMagic.wMagicID == _SKILL_MANWOL || pstMagic->stStdMagic.wMagicID == _SKILL_MANWOL_HIGH )
								{
